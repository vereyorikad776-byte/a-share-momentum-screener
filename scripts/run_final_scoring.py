#!/usr/bin/env python3
"""
预处理脚本：把batch CSV合并并按thscode拆分成单票文件
然后运行完整版v22评分
"""
...
# Step 1: 合并所有batch文件并按thscode拆分
print("Step 1: 合并并拆分K线数据...")
all_batches = []
for batch_file in sorted(glob.glob('/tmp/kline_batch_*.csv')):
    try:
...
print(f"✓ 已拆分 {combined['thscode'].nunique()} 只票到 /tmp/kline_*.csv")
...
print(f"候选池共 {len(candidates)} 只票")
...
for code, info in candidates.items():
    ...
    try:
        ...
        data = {
            ...
            'notice_risk': 1, 'date': '20250818', 'high_recent': high_20d,
...
print(f"\n成功评分 {len(results)} 只票")
print("=" * 80)
print("Top 20 完整版评分榜 (v22_engine 全量86只)")
print("=" * 80)

for i, r in enumerate(results[:20], 1):
    sources = '/'.join(r['sources'])
    print(f"\n{i}. {r['code']} {r['name']} | 来源: {sources}")
...
# 保存
output = {'date': '20250818', 'total_scored': len(results), 'top20': results[:20], 'all_results': results}
with open('/root/.openclaw/workspace/skills/ifind-momentum-screener/data/top20_final_v22.json', 'w') as f:
    json.dump(output, f, ensure_ascii=False, indent=2, default=str)

print(f"\n✓ 最终结果已保存到 top20_final_v22.json")
