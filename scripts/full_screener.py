#!/usr/bin/env python3
"""
full_screener.py — 完整选股流程
iFinD粗筛 → Baostock拉K线 → v2.2精筛
"""
...
def ifind_coarse_screen():
    """iFinD粗筛：涨幅3%-8%（排除涨停），成交>3000万"""
    print("🔍 Step 1: iFinD粗筛（涨幅3%-8%，成交>3000万）...")
    result = call('stock', 'search_stocks', {
        'query': '主板 涨幅大于3 涨幅小于8 成交额大于3000万',
        'limit': 100
    })
        ...
        if line.startswith('|') and '股票代码' not in line and '---' not in line:
            ...
            if len(parts) >= 5:
                try:
                    ...
                    if not name.startswith('*ST') and not name.startswith('ST') and not name.startswith('退市'):
                        stocks.append({'code': code, 'name': name, 'pct_chg': pct_chg, 'amount': amount})
                except:
                    pass

    # 只取前15只（减少Baostock请求量）
    stocks = stocks[:15]
    print(f"✅ iFinD粗筛: {len(stocks)}只（取前15只扫描）")
    return stocks


def fetch_kline_bs(code, days=25):
    """Baostock获取K线"""
    # 转换代码格式
...
def run_v21_full(code, name, tech):
    """跑完整v2.2评分"""
...
def main():
    print("=" * 60)
    print(f"📊 A股动量选股 v2.2 — 完整流程")
    print(f"时间: {datetime.now().strftime('%Y-%m-%d %H:%M')}")
    print("=" * 60)
    print()

    # Step 1: iFinD粗筛
    candidates = ifind_coarse_screen()
    if not candidates:
        print("❌ iFinD无数据")
        return
    print()
    ...
    for i, s in enumerate(candidates[:20], 1):
        print(f"  {i:2d}. {s['code']} {s['name']:10s}: +{s['pct_chg']:5.2f}%  成交¥{s['amount']/10000:8.0f}万")
    print()

    # Step 2: Baostock登录
    print("🔌 Step 2: Baostock获取K线...")
    bs.login()
    print("✅ 已连接")
    print()

    # Step 3: 精筛
    print("🔬 Step 3: v2.2精筛（真实K线+技术指标）...")
    print(f"    扫描 {len(candidates)} 只...")
    print()

    results = []
    for i, s in enumerate(candidates):
        print(f"  [{i+1}/{len(candidates)}] {s['code']} {s['name']}...", end=' ')

        df = fetch_kline_bs(s['code'])
        time.sleep(1.5)  # 防超时
        ...
        result = run_v21_full(s['code'], s['name'], tech)
