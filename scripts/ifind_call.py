...
try:
    ...
    IFIND_CONFIGURED = True
except Exception as e:
    CONFIG = {}
    AUTH_TOKEN = ""
    IFIND_CONFIGURED = False
    print(f"⚠️ iFinD MCP配置未找到: {e}")
    print(f"   如需使用iFinD，请创建 {Path(__file__).resolve().parent / 'mcp_config.json'}")
    print(f"   格式: {{\"auth_token\": \"your_token_here\"}}")
...
def call(server_type, tool_name, params):
    if not IFIND_CONFIGURED:
        return {
            "ok": False,
            "error": "iFinD未配置",
            "message": "请创建mcp_config.json并设置auth_token",
        }
